...
namespace UniVRM10
{
    ...
    public class Vrm10Runtime : IDisposable
    {
        ...
        public Vrm10Runtime(Vrm10Instance instance, bool useControlRig, IVrm10SpringBoneRuntime springBoneRuntime)
        {
            ...
            if (instance == null)
            {
                throw new ArgumentNullException(nameof(instance), "Vrm10Instance is null");
            }

            m_instance = instance;  // ✅ Properly assign here
            var runtime = m_instance.GetComponent<RuntimeGltfInstance>();

            if (runtime == null)
            {
                throw new Exception("RuntimeGltfInstance component is missing from the VRM GameObject.");
            }
            ...
            if (!instance.TryGetBoneTransform(HumanBodyBones.Head, out m_head))
            {
                throw new Exception("Head bone not found in Humanoid rig.");
            }
